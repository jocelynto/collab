...
prep_instr_0 = visual.TextStim(win=win, name='prep_instr_0',
    text='',
    font='Songti SC',
...
prep_instr_0.setPos(txt_loc)
prep_instr_0.setText(rule_txt)
target_0.setPos(im_loc)
target_0.setImage(target_im)
...
# keep track of which components have finished
instructionsComponents = [instr_txt, prepared_resp, prep_instr_0, target_0, trial_fixation_0_1, image_0_1, key_resp_0_1, trial_fixation_0_2, image_0_2, key_resp_0_2, trial_fixation_0_3, image_0_3, key_resp_0_3, trial_fixation_0_4, image_0_4, key_resp_0_4, trial_fixation_0_5, image_0_5, key_resp_0_5, trial_fixation_0_6, image_0_6, key_resp_0_6, trial_fixation_0_7, image_0_7, key_resp_0_7, trial_fixation_0_8, image_0_8, key_resp_0_8, trial_fixation_0_9, image_0_9, key_resp_0_9, trial_fixation_0_10, image_0_10, key_resp_0_10]
...
while continueRoutine:
    ...
    # *prep_instr_0* updates
    
    # if prep_instr_0 is starting this frame...
    if prep_instr_0.status == NOT_STARTED and prepared_continue:
        # keep track of start time/frame for later
        prep_instr_0.frameNStart = frameN  # exact frame index
        prep_instr_0.tStart = t  # local t and not account for scr refresh
        prep_instr_0.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(prep_instr_0, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'prep_instr_0.started')
        # update status
        prep_instr_0.status = STARTED
        prep_instr_0.setAutoDraw(True)
    
    # if prep_instr_0 is active this frame...
    if prep_instr_0.status == STARTED:
        # update params
        pass
    
    # if prep_instr_0 is stopping this frame...
    if prep_instr_0.status == STARTED:
        if frameN >= (prep_instr_0.frameNStart + 150):
            # keep track of stop time/frame for later
            prep_instr_0.tStop = t  # not accounting for scr refresh
            prep_instr_0.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'prep_instr_0.stopped')
            # update status
            prep_instr_0.status = FINISHED
            prep_instr_0.setAutoDraw(False)
    
    # *target_0* updates
    
    # if target_0 is starting this frame...
    if target_0.status == NOT_STARTED and prepared_continue:
        # keep track of start time/frame for later
        target_0.frameNStart = frameN  # exact frame index
        target_0.tStart = t  # local t and not account for scr refresh
        target_0.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(target_0, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'target_0.started')
        # update status
        target_0.status = STARTED
        target_0.setAutoDraw(True)
    
    # if target_0 is active this frame...
    if target_0.status == STARTED:
        # update params
        pass
    
    # if target_0 is stopping this frame...
    if target_0.status == STARTED:
        if frameN >= (target_0.frameNStart + 150):
            # keep track of stop time/frame for later
            target_0.tStop = t  # not accounting for scr refresh
            target_0.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'target_0.stopped')
            # update status
            target_0.status = FINISHED
            target_0.setAutoDraw(False)
    
    # *trial_fixation_0_1* updates
    
    # if trial_fixation_0_1 is starting this frame...
    if trial_fixation_0_1.status == NOT_STARTED and target_0.status==FINISHED:
        # keep track of start time/frame for later
    ...
    if image_0_2.status == NOT_STARTED and key_resp_0_1.status==FINISHED:
        # keep track of start time/frame for later
    ...
    if key_resp_0_2.status == NOT_STARTED and trial_fixiation_0_2.status==FINISHED:
        # keep track of start time/frame for later
...
for thisRun in run:
    ...
    # keep track of which components have finished
    stim_blockComponents = [rest_txt_1, prep_instr_1, target_1, trial_fixation_1_1, image_1_1, trial_fixation_1_2, image_1_2, trial_fixation_1_3, image_1_3, trial_fixation_1_4, image_1_4, trial_fixation_1_5, image_1_5, trial_fixation_1_6, image_1_6, trial_fixation_1_7, image_1_7, trial_fixation_1_8, image_1_8, trial_fixation_1_9, image_1_9, trial_fixation_1_10, image_1_10]
    ...
    while continueRoutine:
        ...
        if target_1.status == NOT_STARTED and rest_txt_1.status==FIINISHED:
            # keep track of start time/frame for later
