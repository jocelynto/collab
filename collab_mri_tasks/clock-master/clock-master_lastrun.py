...
expName = 'clock-master'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jocel\\Documents\\collab_mri_tasks\\clock-master\\clock-master_lastrun.py',
...
for thisBlock in block:
    ...
    use_rows = match_idx[n*5:n*5+5],no_match_idx[n*5:n*5+5]
    ...
    while continueRoutine:
        ...
        if start_indicator.status == NOT_STARTED and frameN >= 120:
            # keep track of start time/frame for later
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('clock_types.xlsx', selection=use_rows),
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if clock_digital.status == NOT_STARTED and frameN >= 0:
                # keep track of start time/frame for later
            ...
            if direction_txt.status == NOT_STARTED and frameN >= 6:
                # keep track of start time/frame for later
        ...
        try:
            if int(key_resp.keys) == int(corrAns):
                thisExp.addData('corr_var', 1)
            elif int(key_resp.keys) != int(corrAns):
        ...
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from iti_trial
