...
expName = 'stroop-practise'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'session': '001',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jocel\\Documents\\Collab project Marco Pang\\stroop-practise\\stroop-practise_lastrun.py',
...
instr_fixation = visual.TextStim(win=win, name='instr_fixation',
    text='+',
    font='Open Sans',
...
# update component parameters for each repeat
# keep track of which components have finished
prepareComponents = [instr_fixation]
...
while continueRoutine and routineTimer.getTime() < 0.5:
    ...
    # *instr_fixation* updates
    
    # if instr_fixation is starting this frame...
    if instr_fixation.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instr_fixation.frameNStart = frameN  # exact frame index
        instr_fixation.tStart = t  # local t and not account for scr refresh
        instr_fixation.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instr_fixation, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'instr_fixation.started')
        # update status
        instr_fixation.status = STARTED
        instr_fixation.setAutoDraw(True)
    
    # if instr_fixation is active this frame...
    if instr_fixation.status == STARTED:
        # update params
        pass
    
    # if instr_fixation is stopping this frame...
    if instr_fixation.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > instr_fixation.tStartRefresh + 0.5-frameTolerance:
            # keep track of stop time/frame for later
            instr_fixation.tStop = t  # not accounting for scr refresh
            instr_fixation.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instr_fixation.stopped')
            # update status
            instr_fixation.status = FINISHED
            instr_fixation.setAutoDraw(False)
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('trialTypes_practise.xlsx'),
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if top_txt.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if top_txt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > top_txt.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if bottom_txt.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        if bottom_txt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > bottom_txt.tStartRefresh + 1.9-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp.tStartRefresh + 1.9-frameTolerance:
                # keep track of stop time/frame for later
